...
import {
  ...
  parseNumberFilter,
  runFormatCommand,
  extractAttachmentAssets,
  processBodyAttachments,
  AttachmentContext,
  run,
} from '../../index';
...
describe('Sync Issues Action', () => {
  ...
  describe('fetchIssueRelationships', () => {
    ...
    it('should return partial results when a later batch fails', async () => {
      ...
      const batch1Response: Record<string, any> = {};
      ...
      expect(core.warning).toHaveBeenCalledWith(
        expect.stringContaining('Transient network error')
      );
    });
    ...
    it('should break and return partial results on schema error in later batch', async () => {
      ...
      mockOctokit.graphql
        ...
        .mockRejectedValueOnce(
          new Error("Field 'parent' doesn't exist on type 'Issue'")
        );
  ...
  describe('runFormatCommand', () => {
    ...
    it('does nothing when there are no modified files', () => {
      ...
      expect(mockInfo).toHaveBeenCalledWith(
        'No modified files; skipping format-command'
      );
    });
    ...
    it('replaces every {files} placeholder with quoted file paths', () => {
      ...
      expect(mockExecSync).toHaveBeenCalledWith(
        "prettier --write 'docs/issues/issue-1.md' 'docs/pull-requests/pr-2.md' && markdownlint 'docs/issues/issue-1.md' 'docs/pull-requests/pr-2.md'",
    ...
    it('wraps execution errors with format-command context', () => {
      ...
      expect(() => runFormatCommand('bad-tool {files}', ['a.md'])).toThrow(
        /format-command failed: exit 2/
      );
    });
  });
  ...
  describe('attachment syncing', () => {
    ...
    const freshCtx = (forceUpdate = false): AttachmentContext => ({
      dir: 'out/attachments',
    ...
    describe('processBodyAttachments', () => {
      ...
      it('downloads file attachments directly with a sanitized filename', async () => {
        ...
        expect(mockWriteFileSync).toHaveBeenCalledWith(
          'out/attachments/12345-debug_run.log',
          expect.anything()
        );
      });
    ...
    describe('run() integration', () => {
      ...
      const setMockOctokit = (mock: unknown) => mockGetOctokit.mockReturnValue(mock as MockOctokit);
      ...
      it('requests full media type, downloads attachments, and reports them in modified-files', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          ...
          if (name === 'sync-attachments') return 'true';
          if (name === 'sync-prs') return 'false';
          if (name === 'sync-sub-issues') return 'false';
          return '';
        });
        mockFetch.mockResolvedValue(okResponse());
        ...
        expect(octokit.rest.issues.get).toHaveBeenCalledWith(
          expect.objectContaining({ mediaType: { format: 'full' } })
        );
        expect(octokit.rest.issues.listComments).toHaveBeenCalledWith(
          expect.objectContaining({ mediaType: { format: 'full' } })
        );
        expect(mockWriteFileSync).toHaveBeenCalledWith(
          `synced-issues/attachments/${UUID}.png`,
          expect.anything()
        );
        const mdWrite = mockWriteFileSync.mock.calls.find(
          (call) => call[0] === 'synced-issues/issues/issue-1.md'
        );
        expect(mdWrite).toBeDefined();
        expect(mdWrite![1]).toContain(`![shot](../attachments/${UUID}.png)`);
        const modifiedFiles = mockSetOutput.mock.calls.find(
          (call) => call[0] === 'modified-files'
        )![1] as string;
      ...
      it('passes only markdown files to the format command', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          ...
          if (name === 'sync-prs') return 'false';
          if (name === 'sync-sub-issues') return 'false';
          if (name === 'format-command') return 'fmt {files}';
          return '';
        });
        mockFetch.mockResolvedValue(okResponse());
        const octokit = buildOctokit();
        ...
        expect(mockExecSync).toHaveBeenCalledWith(
          "fmt 'synced-issues/issues/issue-1.md'",
          expect.objectContaining({ stdio: 'inherit' })
        );
      });
      ...
      it('rewrites attachments in PR bodies and review comments', async () => {
        ...
        const mdWrite = mockWriteFileSync.mock.calls.find(
          (call) => call[0] === 'synced-issues/pull-requests/pr-10.md'
        );
        expect(mdWrite).toBeDefined();
        expect(mdWrite![1]).toContain(`![shot](../attachments/${UUID}.png)`);
      });
  ...
  describe('Input Parameters', () => {
    ...
    describe('token input', () => {
      it('should set outputs for issues-count, prs-count, last-synced-at, and modified-files', async () => {
        ...
        const modifiedFilesCall = mockSetOutput.mock.calls.find(
          (call) => call[0] === 'modified-files'
        );
        expect(modifiedFilesCall).toBeDefined();
        const modifiedFiles = modifiedFilesCall![1] as string;
        expect(modifiedFiles).toContain('synced-issues/issues/issue-1.md');
      ...
      it('should use provided token input', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          if (name === 'token') return 'test-token-123';
          return '';
        });
        delete process.env.GITHUB_TOKEN;
    ...
    describe('output-dir input', () => {
      ...
      it('should use custom output directory when provided', async () => {
        mockGetInput.mockImplementation((name: string) => {
          if (name === 'token') return 'test-token';
          if (name === 'output-dir') return '/custom/output/path';
          return '';
        });
      ...
      it('should not create directory if it already exists', async () => {
        mockGetInput.mockImplementation((name: string) => {
          if (name === 'token') return 'test-token';
          if (name === 'output-dir') return '/existing/path';
          return '';
        });
        mockExistsSync.mockReturnValue(true);
    ...
    describe('sync-issues input', () => {
      ...
      it('should sync issues when set to true', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          if (name === 'token') return 'test-token';
          if (name === 'sync-issues') return 'true';
          return '';
        });
    ...
    describe('updated-since input', () => {
      it('should pass since to issues and filter PRs when provided', async () => {
        ...
        expect(mockOctokit.rest.issues.listForRepo).toHaveBeenCalledWith(
          expect.objectContaining({ since: '2024-02-01T00:00:00Z' })
        );
        expect(mockOctokit.rest.pulls.list).toHaveBeenCalledWith(
      ...
      it('should read updated-since from state file when input missing', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          if (name === 'token') return 'test-token';
          if (name === 'state-file') return '/tmp/state/last.txt';
          return '';
        });

        mockExistsSync.mockImplementation((pathArg: fs.PathLike) => {
          if (String(pathArg) === '/tmp/state/last.txt') return true;
          return false;
        });
    ...
    describe('modified-files output', () => {
      ...
      it('should output modified-files when only issues synced', async () => {
        ...
        expect(mockSetOutput).toHaveBeenCalledWith(
          'modified-files',
          'synced-issues/issues/issue-1.md'
        );
        expect(mockSetOutput).toHaveBeenCalledWith('issues-count', 1);
      ...
      it('should output modified-files when only PRs synced', async () => {
        ...
        expect(mockSetOutput).toHaveBeenCalledWith(
          'modified-files',
          'synced-issues/pull-requests/pr-10.md'
        );
    ...
    describe('sub-issue relationships', () => {
      ...
      it('should write parent and children from GraphQL into issue frontmatter', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          if (name === 'token') return 'test-token';
          if (name === 'sync-prs') return 'false';
          if (name === 'sync-sub-issues') return 'true';
          return '';
        });
    ...
    describe('pagination', () => {
      it('should handle pagination for issues', async () => {
        ...
        expect(mockOctokit.rest.issues.listForRepo).toHaveBeenNthCalledWith(
          1,
    ...
    describe('app token fallback', () => {
      it('should fall back to github token when app token generation fails', async () => {
        ...
        expect(mockSetOutput).toHaveBeenCalledWith('github-token', 'test-token');
        expect(mockWarning).toHaveBeenCalledWith(
          expect.stringContaining('Falling back to provided token')
        );
    ...
    describe('state file handling', () => {
      it('should handle state file read errors gracefully', async () => {
        ...
        expect(mockWarning).toHaveBeenCalledWith(
          expect.stringContaining('Could not read state file')
        );
      ...
      it('should handle state file write errors gracefully', async () => {
        ...
        mockExistsSync.mockReturnValue(false);
        mockWriteFileSync.mockImplementation((pathArg: unknown) => {
          if (String(pathArg) === '/tmp/state/last.txt') {
        ...
        expect(mockWarning).toHaveBeenCalledWith(
          expect.stringContaining('Failed to persist last sync timestamp')
        );
    ...
    describe('custom output directory', () => {
      it('should output correct modified-files paths with custom output directory', async () => {
        mockGetInput.mockImplementation((name: string): string => {
          if (name === 'token') return 'test-token';
          if (name === 'output-dir') return 'custom/docs';
          if (name === 'sync-prs') return 'false';
          return '';
        });
